...
- **Subscription Tier Gating**: Implements 'Starter', 'Pro', and 'Avanzado' tiers with quantity limits (registers, locations, users, warehouses, products, DIAN documents) and feature flags (e.g., user management, advanced inventory, detailed reports, multi-location, e-commerce integrations). Backend enforces limits and features, and the frontend `useSubscription()` hook provides UI gating.
