...
- **DIAN/MATIAS Electronic Billing (Colombia)**: Full integration for electronic invoicing (POS, Invoice, Credit/Debit Notes, Documento Soporte) with asynchronous processing, encrypted credential storage, OAuth token caching, numbering with row-level locks, and a queue-based retry mechanism. Each tenant has unique, encrypted MATIAS API credentials managed in the Admin Console. **Documento Soporte**: UI at `/electronic-billing/support-doc` allows tenants to create DIAN support documents (type_document_id=11) for purchases from non-invoicing suppliers. Supports supplier selection from existing suppliers or manual entry, multiple line items with tax rates (0/5/8/19%), and submits via MATIAS API POST `/ds/document`. Backend route: `POST /api/billing/matias/support-doc`. Fully localized (en/es/pt).
- **Multi-Location Inventory**: Each POS register is linked to a warehouse via `warehouseId`. When a sale is made, stock movements are automatically deducted from the register's assigned warehouse. Register-warehouse assignment is configurable in Settings > Registers.
...
- **WhatsApp/Gupshup Notifications**: A paid add-on for tenants to send and receive WhatsApp notifications. Features include admin-managed message packages with quotas, metered billing, encrypted Gupshup API key storage, and outbound notifications (receipts, low stock alerts) and inbound commands (e.g., RECIBO, HORARIO). All tenants currently use Flowp's centralized WhatsApp account, with global Gupshup credentials managed by the admin. Includes a **WhatsApp Template Manager** where tenants can create message templates with variable placeholders ({{1}}, {{2}}), submit them to Gupshup for WhatsApp approval, sync approval status, and assign approved templates to business event triggers (sale_completed, low_stock_alert, order_ready, payment_received, daily_summary). Database: `whatsapp_templates` and `whatsapp_template_triggers` tables. Backend routes: `/api/whatsapp/templates` (CRUD + submit + sync-status) and `/api/whatsapp/triggers` (CRUD). Frontend: `WhatsAppTemplateManager` component integrated into WhatsApp Settings.
...
- **Report Export (Excel/PDF)**: Enterprise-tier feature (`reports_export` feature flag) allowing export of any report to Excel (.xlsx) or branded PDF. PDFs include tenant branding: company logo, name, address, phone, and tax ID (NIT) in the header. Backend module: `server/report-export.ts` handles generation using `pdfkit` (PDF) and `xlsx` (Excel). Endpoint: `POST /api/reports/export` accepts `{ reportType, reportTitle, dateRange, format, data, language }`. Supports all 12 report types. Admin can toggle `reports_export` as a checkbox feature in the subscription plan creator (admin billing page). Frontend export buttons appear in the reports page header, gated by subscription feature check. Fully localized (en/es/pt).
