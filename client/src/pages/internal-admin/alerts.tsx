...
export default function InternalAdminAlerts() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await internalAdminFetch(`/api/internal-admin/ebilling/alerts?${params.toString()}`);
      return res.json();
    },
  });
  ...
  const ackMutation = useMutation({
    mutationFn: async (alertId: string) => {
      return internalAdminFetch(`/api/internal-admin/ebilling/alerts/${alertId}/acknowledge`, { method: "POST" });
    },
    onSuccess: () => {
